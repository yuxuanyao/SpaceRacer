    ...
    /*
		...
		while (!game_over){
			...
			for (x_pipe = 0; x_pipe < 370;) {
				...
				if (x_pipe<50 && x_pipe>=0) {
					draw_pipe(320-x_pipe, y_pipe, x_pipe, 0x07E0);             
				}
				else if (x_pipe<320 && x_pipe>=50) {
					draw_pipe(320-x_pipe, y_pipe, 50, 0x07E0);
					if(check_game_over ((320-x_pipe), x_bird, y_pipe, y_bird)) {
						//game_over_loop();
						//return 1;
						game_over = 1;
						break;
				...
				else if (x_pipe<370 && x_pipe>=320) {
					draw_pipe(0, y_pipe, 370-x_pipe, 0x07E0);
				}

				draw_bird(x_bird, y_bird++);
				if (y_bird == 225 || y_bird == 0) {
					game_over = 1;
					//return 1;
					break;
				}
				...
				write_char(7, 6, 'E');
				write_char(8, 6, ':');
				write_char(9, 6, ('0'+score/10));
				write_char(10, 6, ('0'+score%10));
